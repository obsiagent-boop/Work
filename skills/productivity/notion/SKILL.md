...
3. Store in `~/.hermes/.env`:
   ...
   ```
4. **Share target pages/databases with the integration** in Notion: page menu `...` → `Connect to` → your integration name. Without this, the API returns 404/empty results for that page even though it exists.

### Mobile (iOS / Android) Setup & Sharing Workflow

**Critical Pitfall:** A newly created integration token (`ntn_...` or `secret_...`) has **zero access** to your Notion workspace by default. Calling `GET/POST /v1/search` will return 0 results until a page is explicitly shared with the integration connection.

**On iOS / Android Mobile App (10 Seconds):**
1. Open the target page in the Notion app (e.g. create a page named `Workspace`).
2. Tap the **`...` (three dots)** in the top-right corner.
3. Scroll down and tap **Connect to** (or **Add Connections**).
4. Select your integration name (e.g., `Anya's connection` / `Hermes Agent`) and tap **Confirm**.

Once connected, `POST /v1/search` immediately returns the parent page ID for `POST /v1/databases` or `POST /v1/pages`.

**Instant Mobile Fallback (CSV Import):**
If a mobile user wants immediate database access on iOS without API setup: generate a CSV file with typed columns (`Title`, `Assignee`, `Priority`, `Status`, `Due Date`), host it on a public HTTPS URL (e.g., Netlify), and instruct the user to tap **`+` (New Page)** → **Import** → **CSV** in the Notion mobile app.
...
- `references/block-types.md` — Detailed block structure specs for Notion pages.
- `references/lead-scraping-notion-slack.md` — Workflow for scraping local business leads without websites and syncing to Notion & Slack.
...
**Critical Pitfall — Parent Object ID Type:**
- When adding an item to a database, `parent` MUST be `{"database_id": "database_uuid"}`, NOT `{"page_id": "..."}`. Using `page_id` with a database ID returns `HTTP 404 Not Found`.
- Property keys in `properties` MUST match the exact property names and types in that database's schema (case-sensitive).
...
curl -s -X POST "https://api.notion.com/v1/pages" \
  ...
  -d '{
    ...
    "properties": {
      "Task Name": {"title": [{"text": {"content": "New Item"}}]},
      "Status": {"select": {"name": "Todo"}},
      "Priority": {"select": {"name": "High"}},
      "Due Date": {"date": {"start": "2026-08-04"}}
    }
  }'
```
